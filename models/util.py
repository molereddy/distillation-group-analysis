...
class SimKD(nn.Module):
    ...
    def forward(self, feat_s, feat_t, cls_t):
        ...
        s_H, t_H = (feat_s.shape[2], feat_s.shape[3]), (feat_t.shape[2], feat_t.shape[3])

        if s_H[0] * s_H[1] >= t_H[0] * t_H[1]:
            source = F.adaptive_avg_pool2d(feat_s, t_H)
            target = feat_t
        else:
            source = feat_s
            target = F.adaptive_avg_pool2d(feat_t, s_H)
        ...
        if len(cls_t) == 1:
            ...
            pred_feat_s = cls_t[0](temp_feat)
        else:
            trans_feat_s = getattr(self, 'transfer')(source)
            temp_feat_s = trans_feat_s
            for i in range(0, len(cls_t) - 1):
                temp_feat_s = cls_t[i](temp_feat_s)
            # Prediction via Teacher Classifier
            #temp_feat_s == last layer input
            temp_feat = self.avg_pool(temp_feat_s)
            temp_feat = temp_feat.view(temp_feat.size(0), -1)
            pred_feat_s = cls_t[-1](temp_feat)#classifier
