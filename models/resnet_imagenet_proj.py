...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.factor = 2#reduction factor
        self.input_dim = 512#for resnet-18
        self.output_dim = 2048#for resnet-50 classifier
        self.intermediate = 2048//self.factor

        self.simkd_module = nn.Sequential(
            conv1x1(self.input_dim, self.intermediate),
            nn.BatchNorm2d(self.intermediate),
            nn.ReLU(inplace=True),
            #conv3x3(t_n//factor, t_n//factor),
            # depthwise convolution
            conv3x3(self.intermediate, self.intermediate, groups=self.intermediate),
            nn.BatchNorm2d(self.intermediate),
            nn.ReLU(inplace=True),
            conv1x1(self.intermediate, self.output_dim),
    ...
    def forward(self, x, is_feat=False):
        ...
        x = self.layer4(x)
        f4 = x
#simkd_projector = SimKD(s_n, t_n, factor = 2)
        x = self.simkd_module(x)
...
def resnet18_proj(pretrained=False, progress=True, **kwargs):#only resnet18 with projector is implemented
